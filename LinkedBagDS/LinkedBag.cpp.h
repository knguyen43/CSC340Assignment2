...
template<class ItemType>
Node<ItemType>* LinkedBag<ItemType>::reversedFindKthItem(const int& k) const {
    // Invalid input: empty bag or k <= 0 or k > itemCount
    if (isEmpty() || k <= 0 || k > itemCount) {
        return nullptr;
    }
    
    // Find the kth item from the end (1-indexed from the end)
    // k=1 returns the last item, k=itemCount returns the first item
    
    int reversedPosition = itemCount - k + 1;
    
    // Now find the item at the reversed position (1-indexed)
    Node<ItemType>* curPtr = headPtr;
    int position = 1;  // Start at position 1 (head)
    
    // Move to the reversed position
    while (position < reversedPosition) {
        curPtr = curPtr->getNext();
        position++;
    }
...
LinkedBag<ItemType>& LinkedBag<ItemType>::operator=(const LinkedBag<ItemType>& aBag) {
    ...
    if (this != &aBag) {
        ...
        // Copy from aBag, similar to copy constructor
        itemCount = aBag.itemCount;
        Node<ItemType>* origChainPtr = aBag.headPtr;  // Points to nodes in original chain
